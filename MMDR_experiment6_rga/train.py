    ...
    for epoch in range(args.start_epoch, args.num_epochs):
        ...
        for i, sample_batched in enumerate(train_loader):
            ...
            if args.model_type in ["SSAN_R"]:
                ...
                loss_all = 10 * binary_loss + constra_loss + adv_loss
                ...
                adv_loss = binary_fuc(domain_invariant, UUID.long())
                loss_all = 10 * binary_loss + constract_loss + 2 * adv_loss
            n = image_x.shape[0]
            binary_loss_record.update(binary_loss.data, n)
            constra_loss_record.update(constract_loss.data, n)
...
def test_video(model, args, test_loader, score_root_path, epoch, name=""):
    ...
    with torch.no_grad():
        ...
        for i, sample_batched in enumerate(test_loader):
            image_x, label, map_x = sample_batched["image_x"].cuda(), sample_batched["label"].cuda(), sample_batched[
                "map_x"].cuda()
            map_score = 0
            for frame_i in range(image_x.shape[1]):
                if args.model_type in ["SSAN_R"]:
                    cls_x1_x1, fea_x1_x1, fea_x1_x2, _ = model(image_x[:, frame_i, :, :, :],
                                                               image_x[:, frame_i, :, :, :])
                    score_norm = torch.softmax(cls_x1_x1, dim=1)[:, 1]
                elif args.model_type in ["SSAN_M"]:
                    pred_map, fea_x1_x1, fea_x1_x2, _ = model(image_x[:, frame_i, :, :, :],
                                                              image_x[:, frame_i, :, :, :])
