        ...
        for phase in ['train', 'test']:
            if phase == 'train':
                ...
                for i, (image, reg_map_sp) in enumerate(dataloader[phase]):
                    ...
                    image = image.transpose(1, 0, 4, 2, 3)
                    image = image.reshape(bsize * 2, imsize, imsize, 3)
                    image2 = F.interpolate(image, [im2size, im2size])
                    image3 = F.interpolate(image, [im3size, im3size])  #

                    # disentangle the spoof traces / step 1
                    reg_map_sp = reg_map_sp.reshape(bsize, imsize, imsize, 3)
                    M, s, b, C, T = gen(image)
                    ...
                    # hard mode / step 3
                    s_hard = s * torch.Tensor(bsize * 2, 1, 1, 1).uniform_(0.1, 0.8)
                    b_hard = b * torch.Tensor(bsize * 2, 1, 1, 1).uniform_(0.1, 0.8)
                    C_hard = C * torch.Tensor(bsize * 2, 1, 1, 1).uniform_(0.1, 0.8)
                    T_hard = T * torch.Tensor(bsize * 2, 1, 1, 1).uniform_(0.1, 0.8)
                    recon_hard1 = (1 - s_hard) * image - b - F.interpolate(C, [imsize, imsize]) - T
                    recon_hard2 = (1 - s) * image - b_hard - F.interpolate(C, [imsize, imsize]) - T
                    recon_hard3 = (1 - s) * image - b - F.interpolateC_hard(C, [imsize, imsize]) - T
                    ...
                    image_a1 = torch.cat([image[:bsize, ...], recon_hard[bsize:, ...]], dim=0).requires_grad_(
                        requires_grad=False)
                    image_a2 = torch.cat([image[:bsize, ...], synth1], dim=0).requires_grad_(requires_grad=False)
                    ...
                    # loss computation
                    d1_rl, _, d1_sl, _ = torch.split(dis_1l, 4)
                    d2_rl, _, d2_sl, _ = torch.split(dis_2l, 4)
                    d3_rl, _, d3_sl, _ = torch.split(dis_3l, 4)
                    _, d1_rs, _, d1_ss = torch.split(dis_1s, 4)
                    _, d2_rs, _, d2_ss = torch.split(dis_2s, 4)
                    _, d3_rs, _, d3_ss = torch.split(dis_3s, 4)

                    # loss for step 1
                    M_li, M_sp = torch.split(M, 2, 0)
                    ...
                    pixel_loss = l1_loss(traces_a[:bsize, ...], trace_warp.requires_grad_(requires_grad=False))
                ...
                for i, (image, im_name, label) in enumerate(dataloader[phase]):
                    ...
                    accuracy = accuracy_score(label, np.where(score > 0.4, 1, -1))
                    if (i + 1) % train_num == 0:
                        log.write(
                ...
                APCER, NPCER, ACER, HTER, AUC = val_model(y_scores, y_labels, num_epochs=num_epochs)
...
def val_model(scores, labels, num_epochs):
...
if __name__ == '__main__':
    ...
    test_list = get_test_data(test_csv)

    train_set = Dataset_Csv_train(config, train_list_li, train_list_sp, transform=None)
    test_set = Dataset_Csv_test(config, test_list, transform=None)


    image_datasets = {}
    # over sampling
    image_datasets['train'] = data.DataLoader(train_set, batch_size=batch_size, **params)
    image_datasets['test'] = data.DataLoader(test_set, batch_size=batch_size, **params)
